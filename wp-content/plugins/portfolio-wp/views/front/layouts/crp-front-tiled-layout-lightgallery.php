...
function crp_infoBox($crp_project){
    ...
    if( (isset($crp_project->title) && $crp_project->title !== '' ) || (isset($crp_project->description) && $crp_project->description !== '' )){
        ...
        if (isset($crp_project->url) && $crp_project->url !== '' ) {
            $desc = $crp_project->url;
            if (strpos($desc, 'http') !== false) {
                $desc = '<a href="' . $desc . '" target="_blank">' . $desc . '</a>';
            }
            $output .= "<p>Url: ".$desc."</p>";
        }
...
<style>
  ...
  .lg-info h4,.lg-info h3,.lg-info h2 {
    ...
    /* Portfolio Options Configuration Goes Here*/
    #gallery .tile:hover{
        cursor: <?php echo $crp_portfolio->options[CRPOption::kMouseType]; ?>;
    }
    ...
    #gallery .tile .caption {
        background-color: <?php echo CRPHelper::hex2rgba($crp_portfolio->options[CRPOption::kTileOverlayColor].$crp_portfolio->options[CRPOption::kTileOverlayOpacity]) ?>;
    }
    ...
    <div id="gallery">
        <div id="ftg-items" class="ftg-items">
            <?php foreach($crp_portfolio->projects as $crp_project): ?>
                <?php
                    /*echo "<pre>";
                    var_dump($crp_project);
                    echo "Title: ";
                    var_dump(CRPHelper::decode2Str($crp_project->title));
                    echo "Description: ";
                    var_dump(CRPHelper::decode2Str($crp_project->description));
                    echo "Url: ";
                    var_dump($crp_project->url);
                    echo "Cover: ";
                    var_dump(CRPHelper::decode2Str($crp_project->cover));
                    // var_dump(CRPHelper::decode2Str($crp_project->pics));
                    echo "</pre>";*/
                ?>
                <div id="crp-tile-<?php echo $crp_project->id?>" class="tile" data-url="<?php echo isset($crp_project->url) ? $crp_project->url : ""?>">
                    ...
                    <a id="<?php echo $crp_project->id ?>" class="tile-inner">
                        <div class="title"><?php echo CRPHelper::decode2Str($crp_project->title); ?></div>
                        ...
                        if ($showTitle || $showDesc) {
                            ...
                            $html .= '<div class="description">' . CRPHelper::decode2Str($crp_project->description) . '</div><div class="url">Url: ' . $crp_project->url . '</div><div class="caption"></div>';
                        }
                        echo $html;
                        ?>
                    </a>
...
<script>
    (function($) {
        ...
        // TODO: Uncomment for automatic tiling of thumbnails
